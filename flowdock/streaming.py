...
class StreamingAPI(object):
    ...
    def __init__(self, personal_api_token, accept=DEFAULT_CONTENT_TYPE):
        ...
        self.flows = None
        self.active = None
        self.headers = {"content-type": DEFAULT_CONTENT_TYPE, "accept": self.accept}
    ...
    def fetch(self, flows, active=None, plain=False):
        assert isinstance(
            flows, (list, str)
        ), "The `flows` argument must be string or list instance."
        assert (
            active in self.ALLOWED_STATUSES
        ), "The `active` argument must be in %s." % str(self.ALLOWED_STATUSES)
        self.flows = flows
        self.active = active
